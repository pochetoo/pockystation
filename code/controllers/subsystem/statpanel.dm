	...
	if (!resumed)
		...
		*/
		...
		global_data = list(
			...
			"Map: [SSmapping.current_map?.map_name || "Loading..."]",
			cached ? "Next Map: [cached.map_name]" : null,
			"Storyteller: [SSgamemode.storyteller ? SSgamemode.storyteller.name : "N/A"][CONFIG_GET(flag/allow_random_events) ? " + Legacy Random Events" : ""]", // BUBBER EDIT ADDITION | VENUS EDIT: Added Legacy Random Events indicator [CONFIG_GET(flag/allow_random_events) ? " + Legacy Random Events" : ""]
			"Round ID: [GLOB.round_id ? GLOB.round_id : "NULL"]",
